...
\newcommand{\NewPageTitle}[1]{
    ...
    % 隐藏当前页面的页眉和页脚（页码），但页码计数器继续递增
    \thispagestyle{empty} 
    
    % 使用 'center' 环境将内容垂直居中和水平居中
    \begin{center} % <-- ⭐️ 关键修改：开始局部居中环境
        \vspace*{\fill}
        
        % \centering 不再需要，因为它已经在 'center' 环境中
        {\Huge \bfseries #1} % 文本作为参数
        
        \vspace*{\fill}
    \end{center} % <-- ⭐️ 关键修改：结束局部居中环境
    
    \clearpage % 确保下一页的内容也从新页开始
}
